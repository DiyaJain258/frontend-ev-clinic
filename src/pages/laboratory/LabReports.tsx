...
import { FiFileText, FiSearch, FiPrinter, FiRefreshCw } from 'react-icons/fi';
...
const LabReports = () => {
    ...
    const downloadReport = async (order: any) => {
        try {
            ...
            try {
                if (rawFindings && rawFindings.trim().startsWith('{')) {
                    ...
                    if (parsed.type === 'pdf_attachment') pdfAttachment = parsed;
                }
            } catch (e) {
            ...
            if (pdfAttachment && pdfAttachment.fileData) {
                ...
                link.download = pdfAttachment.fileName || `lab-report-${order.id}.pdf`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                toast.success('Original PDF report downloaded');
                return;
            }
            ...
            if (rawFindings) {
                ...
                doc.text('Test Results / Remarks:', 15, finalY);

                doc.setFontSize(10);
                doc.setFont('helvetica', 'normal');
                const splitResults = doc.splitTextToSize(rawFindings, 180);
                doc.text(splitResults, 15, finalY + 7);
            }
            ...
            doc.save(`lab-report-${order.patient?.name || 'patient'}-${order.id}.pdf`);
            toast.success('Generated report downloaded');
        } catch (error) {
            console.error("Failed to handle report:", error);
            toast.error("Failed to process report download");
        }
    };
        ...
        <div className="dashboard-container fade-in">
            <div className="page-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                    {selectedClinic?.logo && (
                        <div style={{ width: '64px', height: '64px', borderRadius: '12px', overflow: 'hidden', background: '#fff', border: '1px solid #E2E8F0', padding: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <img
                    ...
                    <div>
                        <h1>{selectedClinic?.name || 'Lab Reports'}</h1>
                        <p>Comprehensive history of patient laboratory tests.</p>
                    </div>
                </div>
            ...
            <div className="content-card">
                <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                    <h2><FiFileText /> Patient Test History</h2>
