...
const PatientRecords = () => {
    ...
    const allRecords = [
        ...
        ...(records.prescriptions || []).map((p: any) => {
            ...
            const meds = data.ordersSnapshot?.filter((o: any) => o.type === 'PHARMACY').map((m: any) => m.testName).join(', ');
            return {
                ...p,
                sourceTable: 'medicalrecord',
        ...
        ...(records.medical_reports || []).map((r: any) => ({
            ...r,
        ...
        ...(records.external_documents || []).map((d: any) => ({
            ...d,
        ...
        .filter((record, index, self) =>
            index === self.findIndex(r => r.id === record.id && r.sourceTable === record.sourceTable)
        )
        .sort((a, b) => {
    ...
    const handlePrint = async () => {
        ...
        // --- CLINIC HEADER ---
        const startY = await addClinicHeader(
            doc,
            selectedRecord.clinic || selectedClinic,
            `${selectedRecord.displayType?.toUpperCase() || 'MEDICAL'} REPORT`
        );
        ...
        doc.text(
            (selectedRecord.displayType?.toUpperCase() || 'DOCUMENT') + ' REPORT',
            pageWidth / 2, yPos + 7,
            { align: 'center' }
        );
        yPos += 16;
        ...
        let parsed: any = null;
        try {
            if (result && typeof result === 'string' && result.trim().startsWith('{')) parsed = JSON.parse(result);
            else if (result && typeof result === 'object') parsed = result;
        } catch (e) { parsed = null; }

        const looksLikeFile = (str: string) => {
        ...
        if (selectedRecord.unifiedType === 'assessment') {
            ...
            if (Array.isArray(rawFields)) templateFields = rawFields;
            else if (typeof rawFields === 'string') { try { templateFields = JSON.parse(rawFields); } catch (e) { templateFields = []; } }

            const formData = selectedRecord.data && typeof selectedRecord.data === 'object'
                ? selectedRecord.data
                : (() => { try { return typeof selectedRecord.data === 'string' ? JSON.parse(selectedRecord.data) : {}; } catch (e) { return {}; } })();
            ...
            const meds = (prescData.ordersSnapshot || []).filter((o: any) => o.type === 'PHARMACY');
            ...
            if (meds.length > 0) {
                ...
                doc.text('Prescribed Medications:', margin, yPos); yPos += 5;
                // Table header
                doc.setFillColor(30, 27, 75);
                ...
                meds.forEach((med: any, i: number) => {
                    ...
                    if (i % 2 === 0) { doc.setFillColor(248, 250, 252); doc.rect(margin, yPos, pageWidth - margin * 2, 7, 'F'); }
            ...
            if (yPos + lines.length * 5 > pageHeight - 30) { doc.addPage(); yPos = 20; }
            doc.text(lines, margin, yPos);
            yPos += lines.length * 5 + 6;
        } else {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text(
            `Report Generated: ${new Date().toLocaleString()} | ${selectedClinic?.name || 'EV Clinic'}`,
            margin, footerY - 3
        );
        doc.text(
            'This is a computer-generated document. No physical signature required.',
            pageWidth - margin, footerY - 3,
            { align: 'right' }
        );
    ...
    return (
        <div className="p-5 fade-in">
                                    ...
                                    <tr key={idx}>
                                        ...
                                        <td style={{ fontWeight: 600 }}>
                                            {item.unifiedType === 'assessment' ? (
                                                item.formtemplate?.name || 'Consultation'
                                            ) : (
                                                <div className="flex flex-col">
                                                    <span>{item.testName}</span>
                                                    {item.unifiedType === 'prescription' && item.prescriptionData?.advice && (
                                                        <small className="text-muted" style={{ fontWeight: 400, fontSize: '0.75rem' }}>
            ...
            >
                {selectedRecord && (
                    <div className="record-details-modal" id="print-container" style={{ padding: '0', background: '#fff', color: '#000', minHeight: '100vh', width: '100%' }}>
                        {/* 1. CLINIC HEADER */}
                        <div style={{ textAlign: 'center', padding: '1.5rem 0', borderBottom: '1.5px solid #000', marginBottom: '1rem' }}>
                            {(selectedRecord.clinic?.logo || selectedClinic?.logo) ? (
                                <img
                                    src={(selectedRecord.clinic?.logo || selectedClinic?.logo).startsWith('http')
                                        ? (selectedRecord.clinic?.logo || selectedClinic?.logo)
                                        : `${API_URL.replace(/\/api$/, '')}${((selectedRecord.clinic?.logo || selectedClinic?.logo) || '').startsWith('/') ? (selectedRecord.clinic?.logo || selectedClinic?.logo) : `/${selectedRecord.clinic?.logo || selectedClinic?.logo}`}`}
                                    alt="Clinic Logo"
                                    style={{ width: '85px', height: '85px', objectFit: 'contain', marginBottom: '8px' }}
                                />
                            ) : (
                                <div style={{ width: '85px', height: '85px', background: '#f1f5f9', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1e293b', fontSize: '32px', fontWeight: 800, margin: '0 auto 8px auto', border: '1px solid #e2e8f0' }}>
                                    {(selectedRecord.clinic?.name || selectedClinic?.name || 'EV').substring(0, 2).toUpperCase()}
                                </div>
                            )}
                            <h1 style={{ margin: '0 0 4px 0', fontSize: '26px', fontWeight: 800 }}>{selectedRecord.clinic?.name || selectedClinic?.name || 'Exclusive Vision Clinic'}</h1>
                        ...
                        <div style={{ padding: '0 1.5rem', marginBottom: '2rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 2rem' }}>
                                ...
                                <div style={{ fontSize: '14px' }}><strong>Facility / Clinic:</strong> {selectedRecord.clinic?.name || selectedClinic?.name || 'EV Clinic'}</div>
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <div className="results-wrapper">
                                            ...
                                            <div style={{ marginBottom: '1.5rem', padding: '12px 16px', background: '#f8fafc', borderRadius: '8px', borderLeft: '4px solid #1E1B4B' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                                                    ...
                                                    {finalFileUrl && (
                                                        <a
                                                            href={finalFileUrl}
                                                            download={(selectedRecord?.testName || 'Document').replace(/[^\w\s\.-]/g, '')}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 600, color: '#2D3BAE', textDecoration: 'none', background: '#EEF2FF', padding: '3px 10px', borderRadius: '20px' }}
                                                        >
                                            ...
                                            {finalFileUrl ? (
                                                <div className="data-content" style={{ marginTop: '1rem', textAlign: 'center', padding: '2rem 1rem', background: '#fff', border: '1px dashed #cbd5e1', borderRadius: '8px' }}>
                                                ...
                                                <div className="data-content" style={{ marginTop: '1rem' }}>
                                                    ...
                                                    {(() => {
                                                        ...
                                                        if (selectedRecord.unifiedType === 'assessment') {
                                                            ...
                                                            if (Array.isArray(rawFields)) {
                                                                ...
                                                                try { templateFields = JSON.parse(rawFields); } catch (e) { templateFields = []; }
                                                            }
                                                            ...
                                                            if (Array.isArray(templateFields) && templateFields.length > 0) {
                                                                const items = templateFields.map((field: any) => {
                                                                    const value = formData[field.id] ?? formData[field.label] ?? formData[field.name];
                                                                    ...
                                                                    return (
                                                                        <div key={field.id} style={{ padding: '12px 16px', background: '#f8fafc', borderRadius: '8px', borderLeft: '3px solid #6366F1' }}>
                                                                ...
                                                                return (
                                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                                        {entries.map(([key, value]: any) => (
                                                                            <div key={key} style={{ padding: '10px 14px', background: '#f8fafc', borderRadius: '8px', borderLeft: '3px solid #6366F1' }}>
                                                        ...
                                                        if (selectedRecord.unifiedType === 'prescription') {
                                                            ...
                                                            return (
                                                                <div>
                                                                    {prescData.diagnosis && (
                                                                        <div style={{ padding: '12px 16px', background: '#fef3c7', borderRadius: '8px', borderLeft: '3px solid #F59E0B', marginBottom: '16px' }}>
                                                                    ...
                                                                    {prescData.notes && (
                                                                        <div style={{ padding: '12px 16px', background: '#f0fdf4', borderRadius: '8px', borderLeft: '3px solid #22C55E', marginBottom: '16px' }}>
                                                                    ...
                                                                    {meds.length === 0 && !prescData.diagnosis && !prescData.notes && (
                                                                        <div style={{ padding: '20px', background: '#f9fafb', borderRadius: '12px', border: '1px dashed #e5e7eb' }}>
                                                        ...
                                                        if (selectedRecord.displayType === 'Radiology' || (parsed && (parsed.findings || parsed.impression))) {
                                                            return (
                                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                                                    ...
                                                                    {(parsed?.impression || parsed?.notes) && (
                                                                        <div>
                        ...
                        <div className="modal-actions no-print" style={{
                            ...
                            <button
                                ...
                                style={{ borderRadius: '10px', padding: '0.8rem 1.8rem' }}
                            >
                                Close Window
                            </button>
                            <button
