...
const PatientBilling = () => {
    ...
    const totalUnpaid = invoices.reduce((sum, inv) => sum + (inv.status === 'Unpaid' ? Number(inv.totalAmount || inv.amount || 0) : 0), 0);
                        ...
                        <tbody>
                            {invoices.length > 0 ? invoices.map((inv: any) => (
                                <tr key={inv.id}>
                                    ...
                                    <td>{inv.clinic?.name || 'EV Clinic'}</td>
                                    <td>
                                        <button
                        ...
                        <div className="modal-body">
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem', background: '#f8fafc', padding: '1rem', borderRadius: '12px' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontWeight: 600 }}>{new Date(selectedInvoice.date).toLocaleDateString()}</div>
                                </div>
                                <div>
                            ...
                            {selectedInvoice.status === 'Unpaid' && (
                                <div style={{ marginTop: '1.5rem', padding: '1rem', background: '#FEF3C7', color: '#92400E', borderRadius: '8px', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                    <FiInfo size={20} />
