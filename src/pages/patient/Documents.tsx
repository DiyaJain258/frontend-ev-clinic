...
const Documents = () => {
    ...
    useEffect(() => {
        const fetchDocuments = async () => {
            try {
                ...
                const formattedInternalDocs = internalDocs.map((d: any) => {
                    let parsedData: any = {};
                    try {
                    ...
                    return {
                        ...
                        clinic: d.clinic?.name || 'EV Clinic',
                        fullData: d
                    };
                });

                const formattedPureDocs = pureDocs.map((d: any) => ({
                    id: d.id,
                ...
                setDocuments([...formattedPureDocs, ...formattedInternalDocs].sort((a, b) =>
    ...
    const generatePDF = async (doc: any, shouldDownload = true) => {
        ...
        const pdf = new jsPDF();
        const clinicInfo = {
            name: doc.clinic || 'EV Clinic Specialist Centre',
        ...
        pdf.text('This is a digitally generated clinical record from the EV Clinic Medical Information System.', 15, pageHeight - 20);
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        setUploading(true);
        try {
            // Find patient ID for the selected clinic
            await patientService.getMyDocuments(); // Refresh to get latest patient associations if needed
            // Actually, we can just pass the clinicId and common patient data to the backend
            // The backend uploadPatientDocument currently expects patientId in body.
            // But we can fetch it or have backend find it by email.

            // For now, let's assume we need to find it from the docRes or similar.
            // Simplified: The backend should ideally find the patient by email and clinicId.

            // Let's check how to get the patientId for the selected clinic.
            const patientDataRes = await patientService.getMyAppointments();
            const appointments = patientDataRes.data?.data || [];
            const patientForClinic = appointments.find((a: any) => a.clinicId === Number(uploadForm.clinicId))?.patientId;

            if (!patientForClinic) {
                // If no appointment, maybe we have it in some other list?
                // Let's try to find it from any existing document or clinic association.
                toast.error('Could not find your patient record for this clinic. Please book an appointment first.');
                setUploading(false);
                return;
            }
            ...
            <div className="content-card">
                ...
                <div className="table-responsive">
                    <table className="data-table">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan={5} className="text-center p-lg">Retrieving documents...</td></tr>
                        ...
                        <form onSubmit={handleUpload}>
                            <div className="form-group" style={{ marginBottom: '1.25rem' }}>
                                ...
                                <select
                                    className="form-control"
                                    required
                                    value={uploadForm.clinicId}
                                    onChange={e => setUploadForm({ ...uploadForm, clinicId: e.target.value })}
                                >
                            ...
                            <div className="form-group" style={{ marginBottom: '1.25rem' }}>
                                ...
                                <input
                                    type="text"
                                    className="form-control"
                                    placeholder="e.g. Lab Results, ID Card"
                                    required
                            ...
                            <div style={{ display: 'flex', gap: '1rem', marginTop: '2rem' }}>
                                ...
                                <button
                                    ...
                                    className="btn btn-primary"
                                    disabled={uploading}
                                    style={{ flex: 1, background: '#2D3BAE', border: 'none' }}
                                >
