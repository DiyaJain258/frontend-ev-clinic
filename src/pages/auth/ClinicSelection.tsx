            ...
            if (isPatient) {
                ...
                if (storedPatientClinics) {
                    try {
                        ...
                        setClinics(patientClinics.map((pc: any) => ({
                            id: pc.id,
                            name: pc.name,
                ...
                <div className="clinics-list mt-lg">
                    {loading ? (
                        ...
                        clinics.map((clinic: any) => (
                            <div
                                key={clinic.id}
                                className={`clinic-option-card ${selecting === clinic.id ? 'selecting' : ''}`}
                                onClick={() => !selecting && handleSelect(clinic)}
                                style={{ opacity: selecting && selecting !== clinic.id ? 0.5 : 1, pointerEvents: selecting ? 'none' : 'auto' }}
                            >
