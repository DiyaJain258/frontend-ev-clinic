...
interface PharmacyOrdersViewProps {
    title: string;
    subtitle: string;
    mode?: 'dashboard' | 'prescriptions';
}

const PharmacyOrdersView = ({ title, subtitle, mode = 'dashboard' }: PharmacyOrdersViewProps) => {
        ...
        if (order.items && Array.isArray(order.items)) {
            calculatedAmount = order.items.reduce((acc: number, item: any) => {
                ...
                if (price === 0 && inventory.length > 0) {
                    ...
                    if (invItem) price = Number(invItem.unitPrice || 0);
                }
    ...
    const handlePrintSlip = async (order: any) => {
        try {
            const doc = new jsPDF();
            let details: any = {};
            try {
                if (typeof order.result === 'string' && order.result.startsWith('{')) details = JSON.parse(order.result);
                else if (typeof order.result === 'object') details = order.result || {};
            } catch (e) { }
            ...
            if (Array.isArray(items)) {
                items.forEach((item: any) => {
                    ...
                    if (invMatch) {
                        if (unitPrice === 0) unitPrice = Number(invMatch.unitPrice || 0);
                        if (sku === '-') sku = invMatch.sku || '-';
                        if (expiry === '-') expiry = invMatch.expiryDate ? new Date(invMatch.expiryDate).toLocaleDateString() : '-';
                        if (!item.medicineName && !item.name) name = invMatch.name;
                    }
            ...
            <div className={`content-section ${mode === 'dashboard' ? 'mt-xl' : 'mt-md'}`}>
                <div className="content-card">
                    <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {mode === 'dashboard' && (
                            <button className="btn btn-secondary btn-sm" onClick={() => navigate('/pharmacy/prescriptions')}>View All</button>
                        )}
                    </div>
                                            ...
                                            <div key={p.id} className="list-item">
                                                <div className="item-main">
                                                    ...
                                                    <p className="item-sub" style={{ fontWeight: 500, color: '#1E293B' }}>
                                                        {p.items?.length
                                                            ? p.items.map((i: any) => i.medicineName || i.name || i.testName).join(', ')
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => isPaid && handleOpenProcess(p)}
                                                    disabled={!isPaid}
                                                    title={!isPaid ? "Direct patient to reception for payment first" : ""}
                                                >
                            ...
                            orders
                                ...
                                .map(p => {
                                    ...
                                    try {
                                        ...
                                        else if (typeof p.result === 'object') details = p.result || {};
                                    } catch (e) { }
                                    return (
                                    ...
                                    <div style={{ marginTop: '1.5rem' }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                            {selectedOrder.items.map((item: any, idx: number) => {
                                                const qty = Number(item.quantity) || 0;
                                                const itemPrice = Number(item.unitPrice || item.price || (inventory.find(inv =>
                                                    inv.id === item.inventoryId ||
                                ...
                                <tbody>
                                    <tr style={{ borderBottom: '1px solid #E2E8F0' }}>
                                        ...
                                        <td style={{ padding: '16px', textAlign: 'right', fontWeight: 'bold', color: '#1E293B' }}>
                                            {selectedOrder.result && (() => { try { const r = JSON.parse(selectedOrder.result); return r.amount; } catch { return null; } })() ? formatMoney(JSON.parse(selectedOrder.result).amount) : 'Pending'}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '2px solid #E2E8F0', paddingTop: '20px' }}>
                                <div style={{ textAlign: 'right' }}>
                                    ...
                                    <p style={{ fontSize: '24px', fontWeight: 'bold', color: '#2563EB' }}>
                                        {selectedOrder.result && (() => { try { const r = JSON.parse(selectedOrder.result); return r.amount; } catch { return null; } })() ? formatMoney(JSON.parse(selectedOrder.result).amount) : '0.00'}
                                    </p>
                                </div>
                            </div>
