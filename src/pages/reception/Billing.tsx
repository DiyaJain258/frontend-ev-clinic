...
const Billing = () => {
    ...
    const { patients, refreshData } = useApp() as any;
    ...
    const [pendingItems, setPendingItems] = useState<{ consultations: any[], orders: any[] }>({ consultations: [], orders: [] });
    ...
    const fetchPendingItems = async (pId: number) => {
        try {
            setLoadingPending(true);
            const res: any = await billingService.getPendingItems(pId);
    ...
    const handleMarkPaid = async () => {
        if (!invoiceToPay || isSubmittingPayment) return;
        try {
    ...
    const handlePrint = async (invoice: any) => {
        try {
            ...
            const clinicRes: any = await clinicService.getClinicDetails();
            ...
            if (invoice.patient?.phone) doc.text(`Phone: ${invoice.patient.phone}`, margin + 5, yPos + 22);
            if (invoice.patient?.email) doc.text(`Email: ${invoice.patient.email}`, margin + 5, yPos + 27);
            ...
            doc.setTextColor(31, 41, 55);
            doc.text(`${symbol}${Number(invoice.totalAmount).toFixed(2)}`, pageWidth - margin, finalY, { align: 'right' });

            finalY += 8;
            doc.setTextColor(107, 114, 128);
            doc.text('Tax (0%):', summaryX, finalY);
            doc.setTextColor(31, 41, 55);
            doc.text(`${symbol}0.00`, pageWidth - margin, finalY, { align: 'right' });
            ...
            doc.text(`${symbol}${Number(invoice.totalAmount).toFixed(2)}`, pageWidth - margin - 2, finalY + 8, { align: 'right' });
        ...
        <div className="reception-dashboard">
            ...
            <div className="section-card card mt-lg">
                ...
                <div className="table-container mt-md">
                    <table className="data-table">
                        ...
                        <tbody>
                            {loadingInvoices ? (
                                <tr><td colSpan={7} className="text-center p-lg">Loading invoices...</td></tr>
                                ...
                                <tr><td colSpan={7} className="text-center p-lg">No invoices found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Generate Consolidated Invoice" size="lg">
                {isSuccess ? (
                    ...
                    <div className="billing-modal-content">
                        ...
                        {selectedPatientId && (
                            <div className="pending-items-section mt-lg">
                                <h3><FiShoppingCart /> Pending Charges</h3>
                                {loadingPending ? (
                                    <p className="p-md text-center">Fetching unpaid services...</p>
                                ) : (
                                    <div className="pending-items-list mt-md">
                                        {[...pendingItems.consultations, ...pendingItems.orders].length > 0 ? (
                                            <table className="data-table">
                                                <thead>
                                                    <tr>
                                                ...
                                                <tbody>
                                                    {[...pendingItems.consultations, ...pendingItems.orders].map((item) => {
                                                        ...
                                                        return (
                                                            <tr key={`${item.type}-${item.id}`} className={isSelected ? 'selected-row' : ''}>
                                                                <td>
                                                                    <input
                                                                        type="checkbox"
                                                                        checked={!!isSelected}
                                            ...
                                            <p className="text-center p-lg text-muted italic">No unpaid charges found for this patient.</p>
                                        )}
                                    </div>
                                )}
                            </div>
                        ...
                        {selectedItems.length > 0 && (
                            <div className="invoice-summary-box mt-xl p-lg" style={{ background: '#F8FAFC', borderRadius: '12px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                                    <span style={{ fontWeight: 600 }}>Total Consolidated Amount:</span>
                                    <span style={{ fontSize: '1.5rem', fontWeight: 800, color: '#2563EB' }}>{formatMoney(totalAmount)}</span>
                                </div>
                                <div className="form-group">
            ...
            <Modal isOpen={isPayModalOpen} onClose={() => setIsPayModalOpen(false)} title="Record Payment" size="sm">
                <div className="payment-modal p-md">
                    ...
                    <div className="modal-actions mt-lg">
                        <button className="btn btn-secondary" onClick={() => setIsPayModalOpen(false)} disabled={isSubmittingPayment}>Cancel</button>
                        <button className="btn btn-primary" onClick={handleMarkPaid} disabled={isSubmittingPayment}>
            ...
            <Modal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} title="Tax Invoice Details" size="lg">
                {selectedInvoice && (
                    <div className="invoice-detail-template" id="invoice-print-area">
                        {/* Premium Header */}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #2D3BAE', paddingBottom: '1.5rem', marginBottom: '2rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                <div style={{ width: '50px', height: '50px', background: '#2D3BAE', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '20px', fontWeight: 800 }}>
                                    {selectedClinic?.name?.substring(0, 2).toUpperCase() || 'EV'}
                                </div>
                                <div>
                                    <h2 style={{ margin: 0, fontSize: '1.25rem', color: '#1e293b' }}>{selectedClinic?.name || 'Exclusive Vision Clinic'}</h2>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '2px solid #2D3BAE', paddingTop: '1.5rem' }}>
                            <div style={{ width: '250px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                    ...
                                    <span style={{ fontWeight: 600 }}>{formatMoney(selectedInvoice.totalAmount)}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                                    <span style={{ color: '#64748b', fontWeight: 500 }}>Tax (0%)</span>
                                    <span style={{ fontWeight: 600 }}>{formatMoney(0)}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', background: '#2D3BAE', borderRadius: '8px', color: '#fff' }}>
                                    ...
                                    <span style={{ fontWeight: 800, fontSize: '1.25rem' }}>{formatMoney(selectedInvoice.totalAmount)}</span>
                                </div>
                            </div>
                        </div>
