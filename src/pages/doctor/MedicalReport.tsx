...
const MedicalReport = () => {
    ...
    const handleDownloadPDF = async () => {
        ...
        if (reportData.template && reportData.template.fields) {
            reportData.template.fields.forEach((field: any) => {
            ...
            <div className="report-paper">
                <header className="paper-header">
                    <div className="clinic-branding">
                        {selectedClinic?.logo && (
                            <img src={selectedClinic.logo.startsWith('http') ? selectedClinic.logo : `${API_URL.replace(/\/api$/, '')}${selectedClinic.logo.startsWith('/') ? selectedClinic.logo : `/${selectedClinic.logo}`}`} alt="Clinic Logo" />
                        )}
                        <div>
                ...
                <section className="patient-section">
                    <div className="patient-info-card">
                        ...
                        <div className="patient-details-grid">
                            ...
                            {(reportData.dynamicData?.companyName || reportData.companyName) && (
                                <div>
                                    <label>Company Name</label>
                                    <p>{reportData.dynamicData?.companyName || reportData.companyName}</p>
                                </div>
                            )}
                            {(reportData.dynamicData?.employeeId || reportData.employeeId) && (
                                <div>
                ...
                <section className="report-body">
                    {reportData.template && reportData.template.fields ? (
                        reportData.template.fields.map((field: any) => {
                            const value = reportData.dynamicData?.[field.id];
                            if (value === undefined || value === null || value === '') return null;
                            return (
                                <div key={field.id} className="field-box" style={{ marginBottom: '24px' }}>
