...
import {
    FiEye, FiEdit2, FiSearch, FiFileText, FiPlus, FiDownload,
    FiUser, FiCalendar, FiX, FiClipboard, FiPackage, FiActivity
} from 'react-icons/fi';
...
const MedicalReportList = () => {
    ...
    const [reportTypes, setReportTypes] = useState<string[]>(['all']);
    ...
    const fetchTemplates = async () => {
        try {
            const response: any = await medicalReportService.getTemplates();
            if (response.success) {
    ...
    const fetchReports = async () => {
        try {
            setLoading(true);
            const response: any = await medicalReportService.getReports();
            if (response.success) {
    ...
    const getMedicines = (report: any): any[] => {
        if (!report) return [];
        const dynamic = report.dynamicData;
        if (dynamic?.prescriptions && Array.isArray(dynamic.prescriptions)) return dynamic.prescriptions;
        if (dynamic?.ordersSnapshot && Array.isArray(dynamic.ordersSnapshot)) {
            return dynamic.ordersSnapshot.filter((o: any) => o.type === 'PHARMACY' || o.type === 'MEDICINE');
        }
        return [];
    };

    // Get all orders (lab, radiology, pharmacy)
    const getAllOrders = (report: any): any[] => {
        if (!report) return [];
        const dynamic = report.dynamicData;
        if (dynamic?.ordersSnapshot && Array.isArray(dynamic.ordersSnapshot)) return dynamic.ordersSnapshot;
        return [];
    };

    // Get dynamic fields to display
    const getDynamicFields = (report: any) => {
        if (!report?.template?.fields || !report?.dynamicData) return [];
        ...
        return fields
            .map((f: any) => ({ ...f, value: report.dynamicData[f.id] }))
        ...
        try {
            ...
            if (fields.length > 0) {
                fields.forEach((field: any) => {
                    ...
                    doc.setFontSize(11); doc.setTextColor(35, 40, 107); doc.setFont('helvetica', 'bold');
                    doc.text(field.label + ':', 15, yPos); yPos += 7;
                    ...
                    if (yPos + lines.length * 6 > pageHeight - 30) { doc.addPage(); yPos = 20; }
                    doc.text(lines, 15, yPos); yPos += lines.length * 6 + 8;
                });
            } else {
                if (report.diagnosisSummary) {
                    doc.setFontSize(11); doc.setTextColor(35, 40, 107); doc.setFont('helvetica', 'bold');
                    doc.text('Diagnosis:', 15, yPos); yPos += 7;
                    doc.setFontSize(10); doc.setTextColor(50, 50, 50); doc.setFont('helvetica', 'normal');
                ...
                if (report.treatmentGiven) {
                    doc.setFontSize(11); doc.setTextColor(35, 40, 107); doc.setFont('helvetica', 'bold');
                    doc.text('Treatment:', 15, yPos); yPos += 7;
                    doc.setFontSize(10); doc.setTextColor(50, 50, 50); doc.setFont('helvetica', 'normal');
            ...
            if (medicines.length > 0) {
                yPos += 5;
                doc.setFontSize(11); doc.setTextColor(35, 40, 107); doc.setFont('helvetica', 'bold');
                doc.text('Prescribed Medicines:', 15, yPos); yPos += 7;
                medicines.forEach((med: any) => {
                    doc.setFontSize(10); doc.setTextColor(50, 50, 50); doc.setFont('helvetica', 'normal');
                    doc.text(`• ${med.testName || med.medicineName || 'Medicine'} ${med.quantity ? `(Qty: ${med.quantity})` : ''}`, 18, yPos); yPos += 6;
                    if (med.details) {
                        const iLines = doc.splitTextToSize(`  ${med.details}`, pageWidth - 40);
                        doc.text(iLines, 20, yPos); yPos += iLines.length * 5 + 2;
                    }
                });
            }
        ...
        <div className="mrl-page">
            ...
            <div className="mrl-stats-grid">
                ...
                <div className="mrl-stat-card">
                    ...
                    <div className="mrl-stat-info">
                        <p>Unique Patients</p>
                        <h3>{new Set(reports.map((r: any) => r.patientId)).size}</h3>
                    </div>
                </div>
                <div className="mrl-stat-card">
                    ...
                    <div className="mrl-stat-info">
                        <p>This Month</p>
                        <h3>{reports.filter((r: any) => new Date(r.reportDate).getMonth() === new Date().getMonth()).length}</h3>
                    </div>
                </div>
                <div className="mrl-stat-card">
                    ...
                    <div className="mrl-stat-info">
                        ...
                        <h3>{reports.filter((r: any) => getMedicines(r).length > 0).length}</h3>
                    </div>
                </div>
            </div>
            ...
            <div className="mrl-controls card">
                <div className="mrl-search-box">
                    ...
                    <input
                        type="text"
                        placeholder="Search by patient name, MRN, or report type..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                    {searchTerm && (
                        <button className="mrl-clear" onClick={() => setSearchTerm('')}><FiX /></button>
                    )}
                </div>
                                        ...
                                        <div className="mrl-empty">
                                            ...
                                            {!searchTerm && (
                                                <button
                                                    ...
                                                    onClick={() => navigate('/doctor/medical-report/new')}
                                                    style={{ background: 'linear-gradient(135deg, #2D3BAE 0%, #1E1B4B 100%)', border: 'none', color: 'white' }}
                                                >
                                    ...
                                    return (
                                        <tr key={report.id}>
                                            ...
                                            <td className="mrl-details-cell">
                                                {previewField ? (
                                                    <span title={String(previewField.value)}>
                                                    ...
                                                    <span title={report.diagnosisSummary}>
                                                        <strong>Diagnosis: </strong>
                                                        {report.diagnosisSummary.length > 60
                                                            ? report.diagnosisSummary.slice(0, 60) + '...'
                                                            : report.diagnosisSummary}
                                                    </span>
                                                ) : <span className="mrl-no-data">—</span>}
                                            </td>
                                            <td>
                                                {medicines.length > 0 ? (
                                                    <div className="mrl-medicines-cell">
                                                        {medicines.slice(0, 2).map((m: any, i: number) => (
                                                            <span key={i} className="mrl-med-tag">
                                                                <FiPackage size={10} /> {m.testName || m.medicineName || 'Med'}
                                                                {m.quantity ? ` ×${m.quantity}` : ''}
                                                            </span>
                                                        ))}
                                                        {medicines.length > 2 && (
                                                            <span className="mrl-more-tag">+{medicines.length - 2} more</span>
                                                        )}
                                                    </div>
                                                ) : (
                                                    <span className="mrl-no-data">None</span>
                                                )}
                                            </td>
                                            <td>
                                            ...
                                            <td className="text-right">
                                                <div className="mrl-actions">
                                                    <button
                                                        className="mrl-action-btn view"
                                                        title="View Full Report"
                                                        onClick={() => { setViewedReport(report); setIsViewOpen(true); }}
                                                    >
                                                        <FiEye />
                                                    </button>
                                                    <button
                                                        className="mrl-action-btn edit"
                                                        title="Edit Report"
                                                        onClick={() => navigate(`/doctor/medical-report/edit/${report.id}`)}
                                                    >
                                                        <FiEdit2 />
                                                    </button>
                                                    <button
                                                        className="mrl-action-btn download"
                                                        title="Download PDF"
                                                        disabled={pdfLoading}
            ...
            {isViewOpen && viewedReport && (
                <div className="mrl-overlay" onClick={() => setIsViewOpen(false)}>
                    <div className="mrl-full-view" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="mrl-fv-header">
                            <div className="mrl-fv-clinic">
                                {selectedClinic?.logo && (
                                    <img
                                        src={selectedClinic.logo.startsWith('http') ? selectedClinic.logo : `${(import.meta.env.VITE_API_URL || '').replace(/\/api$/, '')}${selectedClinic.logo}`}
                                        alt="Clinic"
                                    />
                                )}
                                <div>
                            ...
                            <div className="mrl-fv-actions-top">
                                <button
                                    className="mrl-fv-pdf-btn btn-no-hover"
                                    onClick={() => handleDownloadPDF(viewedReport)}
                                    disabled={pdfLoading}
                                >
                        ...
                        <div className="mrl-fv-body">
                            ...
                            <div className="mrl-fv-info-grid">
                                ...
                                {viewedReport.dynamicData?.companyName && (
                                    <div className="mrl-fv-info-block">
                                        <label>Company</label>
                            ...
                            {/* Dynamic Fields */}
                            {getDynamicFields(viewedReport).length > 0 ? (
                                <div className="mrl-fv-section">
                                    <h4 className="mrl-fv-section-title">
                                        <FiActivity /> Clinical Details
                                    </h4>
                                    <div className="mrl-fv-fields-grid">
                                        {getDynamicFields(viewedReport).map((field: any) => (
                                            <div key={field.id} className={`mrl-fv-field ${field.type === 'textarea' || String(field.value).length > 60 ? 'full' : ''}`}>
                            ...
                            {/* All Orders */}
                            {getAllOrders(viewedReport).length > 0 && (
                                <div className="mrl-fv-section">
                                    <h4 className="mrl-fv-section-title">
                                        <FiPackage /> Prescriptions & Orders
                                    </h4>
                                    <div className="mrl-fv-orders">
                                        {getAllOrders(viewedReport).map((order: any, idx: number) => (
                                            <div key={idx} className="mrl-fv-order-row">
                            ...
                            {getAllOrders(viewedReport).length === 0 && getMedicines(viewedReport).length > 0 && (
                                <div className="mrl-fv-section">
                                    <h4 className="mrl-fv-section-title"><FiPackage /> Prescribed Medicines</h4>
                                    <div className="mrl-fv-orders">
                                        {getMedicines(viewedReport).map((med: any, idx: number) => (
                                            <div key={idx} className="mrl-fv-order-row">
