...
const MedicalReportForm = () => {
    ...
    const fetchReport = async (reportId: number) => {
        try {
            setLoading(true);
            const response: any = await medicalReportService.getReportById(reportId);
            if (response.success) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (dynamicData.companyName) payload.companyName = dynamicData.companyName;
            if (dynamicData.employeeId) payload.employeeId = dynamicData.employeeId;
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            if (displayCompany) doc.text(`Company: ${displayCompany}`, pageWidth / 2, startY + 15);
            ...
            if (template && template.fields) {
                ...
                fields.forEach(f => {
                    ...
                    doc.text(f.label + ':', 15, yPos); yPos += 7;
    ...
    const renderDynamicField = (field: any) => {
        ...
        return (
            <div key={field.id} className={`form-group ${field.type === 'textarea' ? 'full-width' : ''}`}>
                ...
                {field.type === 'textarea' ? (
                    <textarea value={value} onChange={(e) => handleDynamicChange(field.id, e.target.value)} required={field.required} />
                ) : field.type === 'checkbox' ? (
                    ...
                    <input type={field.type} value={value} onChange={(e) => handleDynamicChange(field.id, field.type === 'number' ? Number(e.target.value) : e.target.value)} required={field.required} />
                )}
            </div>
        );
    };
    ...
    return (
        <div className="medical-report-form-container fade-in">
            ...
            <form onSubmit={handleSubmit} className="report-form-card">
                <div className="form-grid">
                    ...
                    <div className="form-group full-width" style={{ backgroundColor: '#f8fafc', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ position: 'relative', marginTop: '8px' }}>
                            <input
                                type="text"
                                placeholder="Start typing to search report type (e.g. Sick Leave, Radiology Result...)"
                                list="template-list"
                ...
                <div className="form-grid">
                    {selectedTemplate ? (
                        selectedTemplate.fields.map(renderDynamicField)
                    ) : (
                        !id ? (
                            <p style={{ gridColumn: '1 / -1', color: '#64748b', fontStyle: 'italic', textAlign: 'center', padding: '20px' }}>
