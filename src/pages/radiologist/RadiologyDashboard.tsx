...
const RadiologyDashboard = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            setStats({
                ...
                imageUploaded: data.filter((o: any) => (o.testStatus || o.status) === 'Image Uploaded').length,
                completedToday: data.filter((o: any) =>
                    ['Completed', 'Published', 'Result Uploaded'].includes(o.testStatus || o.status) &&
    ...
    const handleOpenUploadImage = (order: any) => {
    ...
    const handleOpenUploadReport = (order: any) => {
        setSelectedOrder(order);
        setResultText('');
        setPrice(order.price || 0);
        setIsPaid(order.paymentStatus === 'Paid');
    ...
    const handlePrintReport = async (order: any) => {
        try {
            const doc = new jsPDF();
            let details: any = {};
            try { details = JSON.parse(order.result || '{}'); } catch (e) { console.error(e); details = { notes: order.result }; }
            ...
            if (details.type === 'pdf_attachment') findings = details.notes || 'Note: A PDF attachment was uploaded. Please refer to the attached document for full details.';
            if (details.type === 'image_attachment') findings = 'Note: A Radiographic Image was uploaded. Please review the system for the original image.';
            ...
            <div className="content-section mt-xl" ref={pendingRef}>
                <div className="content-card">
                    ...
                    <div className="table-container mt-md">
                        {loading ? (
                            <div className="p-lg text-center">Loading workflow...</div>
                        ) : (
                            <table className="data-table">
                                ...
                                <tbody>
                                    {orders
                                        .filter((o: any) => ['Pending', 'Image Uploaded'].includes(o.testStatus || o.status))
                                        .filter((o: any) =>
                                                ...
                                                return (
                                                    <tr key={order.id}>
                                                        ...
                                                        <td>
                                                            {!isImageUploaded ? (
                                                                <button
                                                                    className="btn btn-primary btn-xs btn-with-icon"
                                                                    onClick={() => handleOpenUploadImage(order)}
                                                                    title={!isPaidStatus ? "Warning: Payment not received" : ""}
                                                                    style={{ padding: '0.4rem 0.75rem', fontSize: '0.75rem', background: 'linear-gradient(135deg, #2D3BAE 0%, #1E1B4B 100%)', border: 'none', color: 'white' }}
                                                                >
                                                                ...
                                                                <button
                                                                    className="btn btn-primary btn-xs btn-with-icon"
                                                                    onClick={() => handleOpenUploadReport(order)}
                                                                    style={{ padding: '0.4rem 0.75rem', fontSize: '0.75rem', background: 'linear-gradient(135deg, #2D3BAE 0%, #1E1B4B 100%)', border: 'none', color: 'white' }}
                                                                >
                ...
                <div className="content-card">
                    ...
                    <div className="table-container mt-md">
                        <table className="data-table">
                            ...
                            <tbody>
                                {orders
                                    .filter((o: any) => ['Completed', 'Published', 'Result Uploaded'].includes(o.testStatus || o.status))
                                    .filter((o: any) =>
                                        o.patient?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                                        o.testName?.toLowerCase().includes(searchTerm.toLowerCase())
                                    )
                                    ...
                                    orders
                                        ...
                                        .slice(0, 15).map((order: any) => {
                                            let details: any = {};
                                            try { details = JSON.parse(order.result || '{}'); } catch (e) { console.error(e); }
