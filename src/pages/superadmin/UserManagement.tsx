...
const UserManagement = () => {
    ...
    const fetchRegistrations = async () => {
        try {
            ...
            const res = await axios.get(`${API_URL}/super/registrations`, {
    ...
    const handleApprove = async (id: number) => {
        if (!window.confirm('Are you sure you want to approve this registration and create a clinic?')) return;
        try {
            const token = localStorage.getItem('ev_token');
            await axios.post(`${API_URL}/super/registrations/${id}/approve`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleReject = async (id: number) => {
        ...
        try {
            const token = localStorage.getItem('ev_token');
            await axios.post(`${API_URL}/super/registrations/${id}/reject`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
                        ...
                        {registrations.length === 0 ? (
                            ...
                            registrations.map(req => (
                                <tr key={req.id}>
                                    ...
                                    <td>
                                        {req.plan ? (
                                            <span className="status-pill paid" style={{ fontSize: '0.7rem', padding: '0.35rem 0.8rem' }}>
                                                {req.plan.name}
                                            </span>
                                        ) : (
                                            <span className="text-muted text-sm border px-2 py-1 rounded">N/A</span>
                                        )}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        {req.status === 'PENDING' ? (
                                            <div className="action-btns">
                                                <button
                                                    className="action-btn-mini btn-mark-paid"
                                                    title="Approve & Create Clinic"
                                                    onClick={() => handleApprove(req.id)}
                                                >
                                                    <FiCheckCircle />
                                                </button>
                                                <button
                                                    className="action-btn-mini btn-mark-unpaid"
                                            ...
                                            <div className="text-center text-muted text-xs font-bold" style={{ letterSpacing: '0.1em' }}>PROCESSED</div>
                                        )}
                                    </td>
                                </tr>
                            ))
