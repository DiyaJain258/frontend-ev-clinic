                        ...
                        {!isEditMode && clinicForm.subscriptionPlan !== 'Trial' && (
                            <div className="total-preview">
                                <div className="preview-row">
                                    ...
                                    <span>{formatMoney(clinicForm.subscriptionAmount * ((clinicForm as any).numberOfUsers || 1))}</span>
                                </div>
                                <div className="preview-row">
                                    <span>GST ({clinicForm.gstPercent}%):</span>
                                    <span>{formatMoney((clinicForm.subscriptionAmount * ((clinicForm as any).numberOfUsers || 1)) * clinicForm.gstPercent / 100)}</span>
                                </div>
                                <div className="preview-total">
                                    <span>Total Payable:</span>
                                    <span style={{ color: '#2D3BAE' }}>{formatMoney((clinicForm.subscriptionAmount * ((clinicForm as any).numberOfUsers || 1)) * (1 + clinicForm.gstPercent / 100))}</span>
                                </div>
                            </div>
                        )}
                        {!isEditMode && clinicForm.subscriptionPlan === 'Trial' && (
