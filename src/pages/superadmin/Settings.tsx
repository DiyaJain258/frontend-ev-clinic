...
const Settings = () => {
    ...
    const fetchProfile = async () => {
        try {
            ...
            const res = await axios.get(`${API_URL}/super/profile`, {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await axios.patch(`${API_URL}/super/profile`, payload, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (res.data.success) {
                ...
                if (storedUser) {
                    ...
                    u.name = res.data.data.name;
                    u.email = res.data.data.email;
    ...
    return (
        <div className="invoices-page fade-in">
            ...
            <div style={{ maxWidth: '680px' }}>
                            ...
                            {isEditing ? (
                                <div className="search-box-wrap" style={{ padding: '0.75rem 1rem' }}>
                                    <input
                                        ...
                                        style={{ width: '100%', fontSize: '1rem', color: '#1E1B4B', fontWeight: 600 }}
                                    />
                                </div>
                            ) : (
                                <div className="settings-item" style={{ padding: '0.75rem 0', borderBottom: 'none' }}>
                ...
                {isEditing && (
                    <div className="stat-card fade-in" style={{ alignItems: 'stretch', textAlign: 'left', padding: '2rem', marginBottom: '1.5rem' }}>
                ...
                {/* Action Buttons */}
                {isEditing && (
                    <div className="d-flex gap-md fade-in">
                        <button
                            onClick={handleCancel}
                            className="btn btn-secondary"
                            style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '0.875rem' }}
                            disabled={isSaving}
                        >
                            <FiX /> Cancel
                        </button>
            ...
            <div style={{ maxWidth: '900px', marginTop: '2rem' }}>
                <div className="stat-card" style={{ alignItems: 'stretch', textAlign: 'left', padding: '2rem' }}>
                        ...
                        {CURRENCIES.map(cur => {
                            ...
                            return (
                                <button
                                    ...
                                    onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLButtonElement).style.borderColor = '#A5B4FC'; }}
                                    onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLButtonElement).style.borderColor = '#F1F5F9'; }}
                                >
                                    {/* Symbol badge */}
                                    <div style={{
                    ...
                    <div style={{ marginTop: '1.5rem', padding: '1rem 1.25rem', background: '#F8FAFC', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                        ...
                        {[99, 1234.5, 99999, 1250000].map(n => (
                            <span key={n} style={{ background: '#EEF2FF', color: '#2D3BAE', padding: '0.3rem 0.875rem', borderRadius: '20px', fontWeight: 800, fontSize: '0.9375rem', letterSpacing: '-0.01em' }}>
                                {formatMoney(n)}
                            </span>
                        ))}
                    </div>
