...
import {
    FiHome, FiUsers, FiCheckCircle, FiDollarSign, FiActivity,
    FiClock, FiTrendingUp, FiAlertCircle, FiArrowRight, FiGrid,
    FiUserPlus, FiFileText, FiPackage, FiRefreshCw
} from 'react-icons/fi';
    ...
    const fetchDashboard = async () => {
        ...
        try {
            const [statsRes, clinicsRes, regsRes, alertsRes] = await Promise.allSettled([
                axios.get(`${API_URL}/super/dashboard/stats`, { headers }),
                axios.get(`${API_URL}/super/clinics`, { headers }),
                axios.get(`${API_URL}/super/registrations`, { headers }),
                axios.get(`${API_URL}/super/alerts`, { headers }),
            ]);

            if (statsRes.status === 'fulfilled') setStats(statsRes.value.data.data);
            if (clinicsRes.status === 'fulfilled') {
                const clinics = clinicsRes.value.data.data || [];
                    ...
                    {quickActions.map((a, i) => (
                        <button
                            ...
                            onMouseEnter={e => {
                                ...
                                (e.currentTarget as HTMLButtonElement).style.boxShadow = `0 8px 20px ${a.color}20`;
                            }}
                            onMouseLeave={e => {
                ...
                <div style={{ background: '#FFF', borderRadius: '20px', border: '1px solid #F1F5F9', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}>
                    <div style={{ padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #F1F5F9', background: '#F8FAFC' }}>
                        ...
                        <button
                            ...
                            style={{ background: 'none', border: 'none', color: '#2D3BAE', fontWeight: 700, fontSize: '0.8125rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
                        >
                        ...
                        {alerts.slice(0, 4).map((alert: any, i: number) => {
                            ...
                            try {
                                ...
                                parsed = typeof raw === 'string' && raw.trim().startsWith('{')
                                    ? JSON.parse(raw)
                                    : { text: raw };
                            } catch { parsed = { text: alert.message || alert.title || 'System alert' }; }
                                ...
                                <div key={i} className="alert-item">
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontSize: '0.75rem', color: '#64748B', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                                            {alertTypeRaw && (
                                                <span style={{ background: iconBg, color: iconColor, padding: '0.15rem 0.6rem', borderRadius: '20px', fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.06em' }}>
                                                    {alertTypeRaw}
                                                </span>
                                            )}
