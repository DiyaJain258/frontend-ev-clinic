...
const TopBar = ({ onToggleSidebar }: TopBarProps) => {
    ...
    const showPlatformPill = user?.roles?.includes('SUPER_ADMIN');
            ...
            if (isPatient) {
                ...
                if (storedPatientClinics) {
                    try {
                        ...
                        setUserClinics(patientClinics.map((pc: any) => ({
                            id: pc.id,
                            name: pc.name,
                ...
                if (user?.clinics) setUserClinics(user.clinics);
            } else {
    ...
    return (
        <header className={`topbar no-print ${isPatientView ? 'patient-nav' : ''}`}>
