                        ...
                        <div className="profile-content-scroll">
                                ...
                                <div className="profile-tab-content">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                            ...
                                            <input
                                                ...
                                                style={{ padding: '0.4rem 0.8rem', width: 'auto', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '0.9rem' }}
                                            ...
                                            {historyDateFilter && (
                                                <button
                                                    ...
                                                    style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '0.4rem' }}
                                    ...
                                    {profile?.medicalRecords?.length > 0 ? (
                                        <div className="timeline">
                                            {profile.medicalRecords
                                                .filter((record: any) => {
                                                    ...
                                                    if (activeTab !== 'history') {
                                                        ...
                                                        if (record.templateId !== targetTemplateId) keep = false;
                                                    }
                                                    if (historyDateFilter) {
                                                        const recordDate = new Date(record.createdAt).toISOString().split('T')[0];
                                                        if (recordDate !== historyDateFilter) keep = false;
                                                    }
                                                    return keep;
                                                })
                                                .map((record: any) => (
                                            ...
                                            {profile.medicalRecords.filter((record: any) => {
                                                let keep = true;
                                                if (activeTab !== 'history') {
                                                    const targetTemplateId = Number(activeTab.split('-')[1]);
                                                    if (record.templateId !== targetTemplateId) keep = false;
                                                ...
                                                return keep;
                                                    ...
                                                    <div className="empty-profile-state" style={{ padding: '3rem 0' }}>
                                                        <FiClock />
                            ...
                            {activeTab === 'prescriptions' && (
                                <div className="profile-tab-content">
                                    ...
                                    {(() => {
                                        ...
                                        profile?.medicalRecords?.forEach((record: any) => {
                                            ...
                                            if (Array.isArray(snapshot)) {
                                                snapshot.filter((o: any) => o.type === 'PHARMACY' || o.type === 'MEDICINE').forEach((med: any) => {
                                                    allMedicines.push({
                                                        ...med,
                                        ...
                                        profile?.serviceOrders?.filter((o: any) => o.type === 'PHARMACY').forEach((order: any) => {
                                                ...
                                                <div className="results-list">
                                                    {allMedicines
                                                        ...
                                                        .map((med: any, idx: number) => (
                                                            <div key={idx} className="prescription-card-modern">
                            ...
                            {activeTab === 'reports' && (
                                <div className="profile-tab-content">
                                    ...
                                    {(profile?.medical_reports?.length > 0 || profile.documents?.filter((d: any) => d.type === 'REPORT' || (d.isClinical && d.type?.toLowerCase().includes('report'))).length > 0) ? (
                                        <div className="timeline">
                                            {/* New Medical Reports from Separate Table */}
                                            {profile.medical_reports?.map((report: any) => (
                                                <div key={`report-${report.id}`} className="timeline-item">
                                                        ...
                                                        <div className="timeline-body">
                                                            ...
                                                            <div className="mt-sm" style={{ display: 'flex', gap: '8px' }}>
                                                                <button
                                                                    ...
                                                                    onClick={() => window.open(`/doctor/medical-report/${report.id}`, '_blank')}
                                                                >
                                                                    <FiEye /> View Full
                                                                </button>
                                                            </div>
                                            ...
                                            {profile.documents
                                                .filter((d: any) => d.type === 'REPORT' || (d.isClinical && d.type?.toLowerCase().includes('report')))
                                                .map((doc: any) => (
                                                    <div key={`doc-${doc.id}`} className="timeline-item">
                                        ...
                                        <div className="billing-summary-grid">
                                            <div className="billing-stats-row">
                                                <div className="stat-card-mini">
                                                    ...
                                                    <div className="value">{formatMoney(profile.invoices.reduce((sum: number, inv: any) => sum + Number(inv.totalAmount || inv.amount || 0), 0))}</div>
                                                </div>
                                                <div className="stat-card-mini">
                                                    <label>Total Paid</label>
                                                    <div className="value paid">{formatMoney(profile.invoices.reduce((sum: number, inv: any) => sum + (inv.status === 'Paid' ? Number(inv.totalAmount || inv.amount || 0) : 0), 0))}</div>
                                                </div>
                                                <div className="stat-card-mini">
                                                    <label>Outstanding</label>
                                                    <div className="value pending">{formatMoney(profile.invoices.reduce((sum: number, inv: any) => sum + (inv.status !== 'Paid' ? Number(inv.totalAmount || inv.amount || 0) : 0), 0))}</div>
                                                </div>
                                            </div>
                                ...
                                <div className="profile-tab-content">
                                    ...
                                    {profile?.documents?.length > 0 ? (
                                        <div>
                                            ...
                                            {profile.documents.filter((d: any) => d.isClinical).length > 0 && (
                                                <div className="document-section" style={{ marginBottom: '24px' }}>
                                                    <h4 style={{ marginBottom: '12px', color: '#2D3BAE' }}>🗂️ Clinical Records</h4>
                                                    <div className="documents-grid">
                                                        {profile.documents.filter((d: any) => d.isClinical).map((doc: any) => (
                                                            <div key={doc.id} className="document-card">
                                            ...
                                            {/* Report Documents */}
                                            {profile.documents.filter((d: any) => !d.isClinical && d.type === 'REPORT').length > 0 && (
                                                <div className="document-section" style={{ marginBottom: '24px' }}>
                                                    <h4 style={{ marginBottom: '12px', color: '#2D3BAE' }}>📋 Reports</h4>
                                                    <div className="documents-grid">
                                                        {profile.documents.filter((d: any) => !d.isClinical && d.type === 'REPORT').map((doc: any) => (
                                                            <div key={doc.id} className="document-card">
                                                                <div className="document-icon">📋</div>
                                                                <div className="document-info">
                                                                    <h5>{doc.name}</h5>
                                            ...
                                            {profile.documents.filter((d: any) => !d.isClinical && !['PASSPORT', 'ID_CARD', 'REPORT'].includes(d.type)).length > 0 && (
                                                <div className="document-section" style={{ marginBottom: '24px' }}>
                                                    <h4 style={{ marginBottom: '12px', color: '#2D3BAE' }}>📎 Other Attachments</h4>
                                                    <div className="documents-grid">
                                                        {profile.documents.filter((d: any) => !d.isClinical && !['PASSPORT', 'ID_CARD', 'REPORT'].includes(d.type)).map((doc: any) => (
                                                            <div key={doc.id} className="document-card">
                                                                <div className="document-icon">📎</div>
                                                                <div className="document-info">
                                                                    <h5>{doc.name}</h5>
                    ...
                    <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '500px' }}>
                        ...
                        <div className="modal-footer">
                            ...
                            <button
                                ...
                                disabled={fileLoading || !uploadData.url || !uploadData.name}
                                onClick={async () => {
                                    try {
